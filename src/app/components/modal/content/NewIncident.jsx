...
export default function NewIncident() {
  ...
  const handleSubmit = () => {
    ...
    if (!form.type) return alert("La categoría es obligatoria");
    if (!form.dueDate) return alert("La fecha de vencimiento es obligatoria");
    if (!form.project) return alert("El proyecto es obligatorio");
    if (assignees.length === 0) return alert("Agrega al menos un asignado");
    if (!location.lat || !location.lng)
      return alert("La ubicación es obligatoria");
    const newIncident = {
      ...form,
      id: crypto.randomUUID(),
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="px-6 py-5 flex flex-col gap-5 overflow-y-auto">
          ...
          <Field label="Asignados" required>
            <select
              value=""
              onChange={(e) =>
                onChangeSelect(e, people, assignees, addAssignee)
              }
              className={inputCls}
            >
              <option value="" disabled>
          ...
          <Field label="Observadores" required>
            <select
              value=""
              onChange={(e) =>
                onChangeSelect(e, people, observers, addObserver)
              }
              className={inputCls}
            >
              <option value="" disabled>
        ...
        <div className="px-6 py-4 border-t border-neutral-200 dark:border-neutral-700 flex justify-end gap-2.5">
          <button
            ...
            className="px-5 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-400 cursor-pointer transition-colors flex items-center gap-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-4 h-4"
